...
import { ChevronLeft, CalendarClock, Mail, MessageCircle, Phone } from "lucide-react"
import {
	Flex, Grid, GridItem, Icon, VStack, Heading, Box,
	Image, Text, HStack, Badge, List, Link, Field, Input,
	InputGroup, Textarea, Button
			...
			<Grid templateColumns={{ base: "1fr", md: "repeat(2, 1fr)" }} gap={{ base: '4', md: '6' }}>
				<GridItem {...cardStyle}>
					<VStack gap="4" align="stretch">
						...
						<Flex
							...
							borderRadius="6px"
						>
							<Box background="gray.100" w={{ base: "full", sm: "33.3%", md: "full", lg: "185px" }} aspectRatio={1} position="relative">
								<Image loading="lazy" src="https://i.pravatar.cc/300" position="absolute" top="0" left="0" w="full" h="full" objectFit="cover" />
							</Box>
						...
						<VStack align="stretch" gap="1">
							...
							<VStack align="stretch" gap="3" maxW={{ xl: "70%" }}>
								<Text color="gray.600" fontSize="sm" lineHeight="20px">ARTPLAY центр дизайна, Москва, 105120, Нижняя Сыромятническая, д. 10 стр. 2</Text>
								<List.Root
									display="flex"
								...
								>
									...
									<List.Item display="flex" alignItems="center" gap="1.5">
										...
										<Link color="inherit" href="tel:+74957977251">+7 495 797-72-51</Link>
									</List.Item>
									<List.Item display="flex" alignItems="center" gap="1.5">
										<Icon as={MessageCircle} boxSize="16px" />
										<Link color="inherit" href="tel:+74957470564">+7 495 747-05-64</Link>
									</List.Item>
									<List.Item display="flex" alignItems="center" gap="1.5">
										<Icon as={Mail} boxSize="16px" />
										<Link color="inherit" href="mailto:order@technolight.ru">order@technolight.ru</Link>
									</List.Item>
								</List.Root>
							</VStack>
						</VStack>
				...
				<GridItem {...cardStyle}>
					<VStack gap="4" align="flex-start">
						...
						<Field.Root>
							<Field.Label fontWeight="semibold" fontSize={{ base: '12px', lg: '14px' }}>
								Email
							</Field.Label>
							<InputGroup>
								<Input name="email" placeholder="username@username.com" {...inputStyle} />
							</InputGroup>
						...
						{/* Для того чтобы показать ошибку, добавляем аттрибут invalid к узлу <Field.Root/> */}
						<Field.Root>
							<Field.Label fontWeight="semibold" fontSize={{ base: '12px', lg: '14px' }}>Текст сообщения</Field.Label>
							<Textarea {...textareaStyle} placeholder="Добрый день! ..." />
							<Field.HelperText>Коротко опишите ваш вопрос или проблему</Field.HelperText>
							<Field.ErrorText>Сообщение об ошибке</Field.ErrorText>
						</Field.Root>
